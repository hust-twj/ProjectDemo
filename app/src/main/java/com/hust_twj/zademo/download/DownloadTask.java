...
public class DownloadTask extends Handler {
    ...
    private static final String TAG = "twj-DownloadTask";
    ...
    private AtomicInteger childCancelCount = new AtomicInteger(0);//子线程取消数量
    private AtomicInteger childPauseCount = new AtomicInteger(0);//子线程暂停数量
    private AtomicInteger childFinishCount = new AtomicInteger(0);//子线程完成数量
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case MSG_PROGRESS://进度
                ...
                float realProgress = progress * 1.0f / mFileLength;
                mListener.onProgress(realProgress);
                LogUtils.e(TAG, "realProgress " + realProgress);
                break;
            case MSG_PAUSE://暂停
    ...
    private void download(final long startIndex, final long endIndex, final int threadId) throws IOException {
        ...
        mHttpUtil.downloadFileByRange(mPoint.getUrl(), finalStartIndex, endIndex, new okhttp3.Callback() {
            @Override
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                LogUtils.e(TAG, "download: " + response.code() + "\t isDownloading:" + isDownloading + "\t" + mPoint.getUrl());
